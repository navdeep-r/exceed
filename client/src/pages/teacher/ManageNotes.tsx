      ...
      {(activeTab === 'pending' || (activeTab === 'notes' && unrefined.length > 0)) && unrefined.length > 0 && (
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-[14px] font-semibold text-white">Ready for AI Refinement</h2>
            <span className="text-[11px] px-1.5 py-0.5 rounded-md bg-amber-500/15 text-amber-400 font-medium ml-1">{unrefined.length}</span>
          </div>
            ...
            {unrefined.map(l => (
              <div key={l.id} className="flex items-center gap-4 p-3.5 rounded-xl transition-colors hover:bg-white/[0.02]"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-gray-200 truncate">{l.title}</p>
                  <p className="text-[11px] text-gray-500">{new Date(l.recorded_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
                </div>
                <button
                  onClick={() => handleRefine(l.id, l.transcript)}
                  disabled={refiningId === l.id}
                  className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[12px] font-semibold text-white disabled:opacity-50 transition-all hover:scale-[1.02]"
        ...
        <div className="space-y-2">
          {(activeTab === 'published' ? filteredNotes.filter((n: any) => n.published_at) : filteredNotes).length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 rounded-2xl"
              ...
              <p className="text-[14px] text-gray-300 font-medium">No notes yet</p>
              <p className="text-[12px] text-gray-600 mt-1">Record a lecture and refine the transcript</p>
            </div>
          ) : (
            (activeTab === 'published' ? filteredNotes.filter((n: any) => n.published_at) : filteredNotes).map((n: any, i: number) => (
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-medium text-gray-200 truncate group-hover:text-white transition-colors">{n.title || 'Untitled Notes'}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[11px] text-gray-500 flex items-center gap-1">
                        <Globe size={10} />
                        {n.language?.toUpperCase() || 'EN'}
                      </span>
                      <span className="text-[11px] text-gray-600">v{n.version || 1}</span>
                    </div>
                  </div>
